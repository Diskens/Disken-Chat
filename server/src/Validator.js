...
class Validator {
  ...
  roomExists(data, reversed) {
    return {success: data.rooms.length == 1,
      reason: reversed? 'Room already exists' : 'Room does not exist'
    };
  }
  validRoomName(data, reversed) {
    let rc = global.config.user;
    let {name} = data;
    return this.composite(
      {name:'Room name', str:name, min:rc.nameMin, max:rc.nameMax},
      ['isOfLength', 'isAlphanumerical']
    );
  }
  passcode(data, reversed) {
    let {room, passcode} = data;
    return {success: room.passcode == passcode,
